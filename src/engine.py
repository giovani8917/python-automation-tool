...
import pyautogui
import keyboard as kb  # For hotkeys
from .models import DemoItem, WildcardItem
from .utils import check_internet_connection, check_battery_level, parse_fecha
...
class AutomationEngine:
    def __init__(self, callback_queue=None):
        ...
        self.MODS = {"ctrl_l", "ctrl_r", "ctrl", "alt_l", "alt_r", "alt", "shift", "shift_r", "shift_l", "win", "windows"}
    ...
    def stop_recording(self, remove_last_seconds=0):
        self.recording = False
        if self.listener_mouse: self.listener_mouse.stop()
        if self.listener_keyboard: self.listener_keyboard.stop()
        ...
        if self.callback_queue:
            ...
            self.emit_log(f"Fin Grabación: {len(final_data)} eventos ({moves} moves, {clicks} clicks)")
    ...
    def start_replay(self, sequence, speed_factor=1.0, infinite=False, repeat_times=1):
        ...
        # Run in thread
        threading.Thread(target=self._replay_loop, args=(sequence, speed_factor, infinite, repeat_times)).start()
            ...
            if event_type == "move":
                ...
                if dragging:
                    if drag_start_position:
                        dist = ((current_x - drag_start_position[0])**2 + (current_y - drag_start_position[1])**2)**0.5
                        if dist < 20:
    ...
    def _convert_key(self, key_name):
        ...
        special_keys = {
            ...
            'Key.cmd': 'command', 'Key.cmd_r': 'command',
            ...
            'ctrl_l': 'ctrl', 'ctrl_r': 'ctrl', # V9 maps both to 'ctrl'
            'alt_l': 'alt', 'alt_r': 'alt', 
        ...
        if not self.global_listener:
            try:
                self.global_listener = keyboard.GlobalHotKeys({
                    '<ctrl>+<shift>+p': self.toggle_pause,
                    '<ctrl>+<shift>+d': self.stop_recording
                })
